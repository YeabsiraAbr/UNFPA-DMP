...
export default function ClinicsPage() {
  ...
  return (
    <DashboardLayout
      title={t("appCopy.shell.clinicsTitle")}
      subtitle={t("appCopy.shell.clinicsSubtitle")}
    >
      {/* Stats */}
        ...
        <Card className="p-4 bg-gradient-to-br from-brand-500 to-brand-600 text-white">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm opacity-80">{t("clinicsPage.totalFacilities")}</p>
            </div>
          </div>
        </Card>
        <Card className="p-4 bg-gradient-to-br from-emerald-500 to-emerald-600 text-white">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm opacity-80">{t("clinicsPage.mobileUnits")}</p>
            </div>
          </div>
        </Card>
        <Card className="p-4 bg-gradient-to-br from-purple-500 to-purple-600 text-white">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm opacity-80">{t("clinicsPage.fixedFacilities")}</p>
            </div>
          </div>
        </Card>
        <Card className="p-4 bg-gradient-to-br from-amber-500 to-amber-600 text-white">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm opacity-80">{t("clinicsPage.totalPatients")}</p>
            </div>
          </div>
        </Card>
      </div>
      ...
      <Card className="mb-6 p-4">
        <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
          <div className="w-64">
            <Input
              placeholder={t("appCopy.search.clinics")}
              value={searchTerm}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredClinics.length === 0 && (
          <div className="col-span-full text-center py-16 text-slate-500">
            ...
            <p>{clinics.length === 0 ? t("clinicsPage.noClinicsAvailable") : t("clinicsPage.noClinicsMatchSearch")}</p>
          </div>
        )}
            ...
            <CardContent className="p-6">
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800 text-center">
                  ...
                  <p className="text-xs text-slate-500">{t("clinicsPage.patientsShortLabel")}</p>
                </div>
                <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800 text-center">
                  <p className="text-2xl font-bold text-slate-900 dark:text-white">
                    {clinic.staff.length}
                  </p>
                  <p className="text-xs text-slate-500">{t("clinicsPage.staff")}</p>
                </div>
              </div>
      ...
      >
        {selectedClinic && (
          <div className="space-y-6">
            ...
            {/* Location Details */}
            <div>
              <h4 className="font-semibold mb-3">{t("clinicsPage.locationDetails")}</h4>
              <div className="grid grid-cols-2 gap-4">
                <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800">
                  <p className="text-xs text-slate-500">{t("clinicsPage.region")}</p>
                  <p className="font-medium">{selectedClinic.region}</p>
                </div>
                <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800">
                  <p className="text-xs text-slate-500">{t("clinicsPage.zone")}</p>
                  <p className="font-medium">{selectedClinic.zone}</p>
                </div>
                <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800">
                  <p className="text-xs text-slate-500">{t("clinicsPage.woreda")}</p>
                  <p className="font-medium">{selectedClinic.woreda}</p>
                </div>
                <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800">
                  <p className="text-xs text-slate-500">{t("clinicsPage.location")}</p>
            ...
            {/* Stats */}
            <div>
              <h4 className="font-semibold mb-3">{t("clinicsPage.statistics")}</h4>
              <div className="grid grid-cols-3 gap-4">
                <div className="p-4 rounded-lg bg-brand-50 dark:bg-brand-900/20 text-center">
                  ...
                  <p className="text-sm text-slate-500">{t("clinicsPage.totalPatients")}</p>
                </div>
                <div className="p-4 rounded-lg bg-emerald-50 dark:bg-emerald-900/20 text-center">
                  ...
                  <p className="text-sm text-slate-500">{t("clinicsPage.staffMembers")}</p>
                </div>
                <div className="p-4 rounded-lg bg-purple-50 dark:bg-purple-900/20 text-center">
                  <p className="text-3xl font-bold text-purple-600">42</p>
                  <p className="text-sm text-slate-500">{t("clinicsPage.visitsPerMonth")}</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 rounded-lg bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800">
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium">{t("clinicsPage.lastSynchronized")}</span>
            ...
            <div className="flex gap-3 pt-4 border-t border-slate-200 dark:border-slate-700">
              <Button variant="primary">
                ...
                {t("clinicsPage.editClinic")}
              </Button>
              <Button variant="outline">
                <Settings className="w-4 h-4" />
                {t("clinicsPage.configure")}
              </Button>
              <Button variant="ghost" className="text-red-600">
                <Trash2 className="w-4 h-4" />
                {t("clinicsPage.deactivate")}
              </Button>
            </div>
          </div>
        )}
